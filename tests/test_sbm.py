"""
Script to evaluate a trained model checkpoint on Stochastic Block Model (SBM) graphs.

Usage:
    python tests/test_sbm.py --ckpt path/to/model.ckpt
"""
...
# Monkeypatch O(E) sparse data loading from test_obgn_arxiv_large
import tests.test_obgn_arxiv_large as loader_patch
...
from tests.utils.sbm_loader import sbm_graph_generator
...
# Monkeypatch Sampler to support our SBM generator
from src.utils.graph_generation import patched_create_graph as existing_patched_create

def sbm_aware_create_graph(self, n, weighted, directed, low=0.0, high=1.0, **kwargs):
    if self._graph_generator == "sbm":
        n_val = self._select_parameter(n)
        return sbm_graph_generator(n_val, seed=None)
    else:
        return existing_patched_create(self, n, weighted, directed, low=low, high=high, **kwargs)

Sampler._create_graph = sbm_aware_create_graph
...
def main():
    ...
    parser.add_argument("--num-samples", type=int, default=15, help="Samples to generate per magnitude")
    ...
    # Override TEST datasets to run SBM
    cfg.DATA.TEST.NUM_SAMPLES = args.num_samples
    cfg.DATA.TEST.GRAPH_GENERATOR = ["sbm", "sbm", "sbm", "sbm"]
    cfg.DATA.TEST.NICKNAME = ["sbm_16", "sbm_80", "sbm_800", "sbm_1600"]
    ...
    # Load Data
    logger.info("Loading test datasets (SBM)...")
    ...
    # Run Eval
    logger.info("Running standard evaluation on SBM...")
    results = trainer.test(model, datamodule=datamodule)

    # Print Table
    print("\n" + "="*80)
    print("SBM EVALUATION RESULTS")
    print("="*80)

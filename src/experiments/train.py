"""
Algorithm-agnostic training pipeline for SALSA-CLRS tasks.

This module contains the shared training infrastructure:
- `train()` — the main training loop with NaN recovery, checkpointing, and testing
...
class EpochProfilingCallback(pl.Callback):
    """Logs average epoch time and throughput (it/s) every N epochs."""
...
def train(model, datamodule, cfg, specs, seed=42, checkpoint_dir=None, enable_wandb=False, enable_progress_bar=False, fast_dev_run=False):
    ...
    if cfg.TRAIN.ENABLE:
        ...
        # Create a dummy DataLoader that just yields our pre-made batches
        class StaticLoader:
            def __iter__(self):
        ...
        # Overwrite the datamodule's train loader
...
def patch_datamodule(datamodule, cfg):
    """Patch SALSACLRSDataModule to handle persistent_workers and pin_memory correctly."""
...
def build_arg_parser():
    """Create the standard argument parser for training scripts."""
...
def setup_and_train(args=None):
    """
    End-to-end training: parse CLI args, load datasets, build model, train, test.

    This is the single entry point used by all per-algorithm training scripts.
    The algorithm is determined entirely by the config file (cfg.ALGORITHM).
    """
    ...
    logger.info(f"Starting {cfg.ALGORITHM} run: {cfg.RUN_NAME}")
    ...
    patch_datamodule(datamodule, cfg)

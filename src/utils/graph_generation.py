...
def get_bfs_dataset(split: str, root: str = "data/salsaclrs"):
    """
    Load the SALSA-CLRS BFS dataset for a specific split.

    This function wraps salsaclrs.load_dataset to specifically fetch BFS data.
    
    Args:
        split (str): One of 'train', 'val', 'test'.
        root (str): The local directory where the dataset should be stored/downloaded.
                    Defaults to 'data/salsaclrs'.
    ...
    Returns:
        SALSACLRSDataset or dict: 
            ...
            - For 'test' split, returns a dictionary mapping graph generator names 
              (e.g., 'er', 'ws') to SALSACLRSDataset objects.
    """
    ...
    print(f"Loading BFS dataset for split: {split}...")
    
    try:
        dataset = load_dataset(algorithm="bfs", split=split, local_dir=root)
